		...
		struct nand_bbt_descr *td, int offs)
{
	...
	while (totlen) {
		...
		if (res < 0) {
			if (retlen != len) {
				printf("nand_bbt: Error reading bad block table\n");
				return res;
			}
			printf("nand_bbt: ECC error while reading bad block table\n");
		}
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < 8; j += bits, act += 2) {
				...
				if (reserved_block_code && (tmp == reserved_block_code)) {
					printf("nand_read_bbt: Reserved block at 0x%012llx\n",
		     ...
		     int chipsel)
{
	...
	write:
		...
		if (td->options & NAND_BBT_SAVECONTENT) {
			...
			if (res < 0) {
				...
				printf("nand_bbt: ECC error while reading block for writing bad block table\n");
			}
			/* Read oob data */
		...
		printf("Bad block table written to 0x%012llx, version "
 ...
 outerr:
	printf("nand_bbt: Error while writing bad block table %d\n", res);
	return res;
}
...
int nand_scan_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	...
	if (!this->bbt) {
		printf("nand_scan_bbt: Out of memory\n");
		return -ENOMEM;
	}
...
static int nand_create_default_bbt_descr(struct nand_chip *this)
{
	...
	if (this->badblock_pattern) {
		printf("BBT descr already allocated; not replacing.\n");
		return -EINVAL;
	}
	bd = yaffs_malloc(sizeof(*bd));
	if (!bd) {
		printf("nand_create_default_bbt_descr: Out of memory\n");
		return -ENOMEM;
	}
...
int nand_default_bbt(struct mtd_info *mtd)
{
    ...
    /* agand_flashbased */
    agand_flashbased.options = NAND_BBT_SCANEMPTY | NAND_BBT_SCANALLPAGES;
    agand_flashbased.offs = 0x20;
    agand_flashbased.len = 6;
    agand_flashbased.pattern = scan_agand_pattern;
    ...
    bbt_main_no_bbt_descr.options = NAND_BBT_LASTBLOCK|NAND_BBT_CREATE|NAND_BBT_WRITE|NAND_BBT_2BIT|NAND_BBT_VERSION|NAND_BBT_PERCHIP|NAND_BBT_NO_OOB;
	bbt_main_no_bbt_descr.len = 4;
	bbt_main_no_bbt_descr.veroffs = 4;
	bbt_main_no_bbt_descr.maxblocks = 4;
	bbt_main_no_bbt_descr.pattern = bbt_pattern;

    /* bbt_mirror_no_bbt_descr */
    bbt_mirror_no_bbt_descr.options = NAND_BBT_LASTBLOCK|NAND_BBT_CREATE|NAND_BBT_WRITE|NAND_BBT_2BIT|NAND_BBT_VERSION|NAND_BBT_PERCHIP|NAND_BBT_NO_OOB;
	bbt_mirror_no_bbt_descr.len = 4;
	bbt_mirror_no_bbt_descr.veroffs = 4;
	bbt_mirror_no_bbt_descr.maxblocks = 4;
	bbt_mirror_no_bbt_descr.pattern = mirror_pattern;
