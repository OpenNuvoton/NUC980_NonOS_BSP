...
static __inline u32 calc_shifts_ceiling(u32 x)
{
	int extra_bits;
	int shifts;
...
static __inline u32 calc_shifts(u32 x)
{
	u32 shifts;
...
// 		buf = kmalloc(dev->param.total_bytes_per_chunk, GFP_NOFS);
		buf = yaffs_malloc(dev->param.total_bytes_per_chunk);
...
// 	return kmalloc(dev->data_bytes_per_chunk, GFP_NOFS);
	return (u8 *)yaffs_malloc(dev->data_bytes_per_chunk);
...
static __inline int yaffs_hash_fn(int n)
{
	if (n < 0)
		n = -n;
...
// 	new_str = kmalloc((len + 1) * sizeof(YCHAR), GFP_NOFS);
	new_str = (YCHAR *)yaffs_malloc((len + 1) * sizeof(YCHAR));
	if (new_str) {
...
// 	dev->block_info = kmalloc(n_blocks * sizeof(struct yaffs_block_info), GFP_NOFS);
	dev->block_info = yaffs_malloc(n_blocks * sizeof(struct yaffs_block_info));
	if (!dev->block_info) {
...
// 	dev->chunk_bits = kmalloc(dev->chunk_bit_stride * n_blocks, GFP_NOFS);
	dev->chunk_bits = yaffs_malloc(dev->chunk_bit_stride * n_blocks);
	if (!dev->chunk_bits) {
...
static __inline int yaffs_gc_process_chunk(struct yaffs_dev *dev,
					struct yaffs_block_info *bi,
...
{
	yaffs_free(in->variant.symlink_variant.alias);
...
// 		dev->cache = kmalloc(cache_bytes, GFP_NOFS);
		dev->cache = yaffs_malloc(cache_bytes);
...
// 			dev->cache[i].data = buf = kmalloc(dev->param.total_bytes_per_chunk, GFP_NOFS);
			dev->cache[i].data = buf = yaffs_malloc(dev->param.total_bytes_per_chunk);
		}
		if (!buf)
			init_failed = 1;
...
// 		dev->gc_cleanup_list = kmalloc(dev->param.chunks_per_block * sizeof(u32), GFP_NOFS);
		dev->gc_cleanup_list = yaffs_malloc(dev->param.chunks_per_block * sizeof(u32));
...
void yaffs_deinitialise(struct yaffs_dev *dev)
{
	if (dev->is_mounted) {
		...
		if (dev->param.n_caches > 0 && dev->cache) {
			...
			for (i = 0; i < dev->param.n_caches; i++) {
				yaffs_free(dev->cache[i].data);
				dev->cache[i].data = NULL;
			}

			yaffs_free(dev->cache);
			dev->cache = NULL;
		}

		yaffs_free(dev->gc_cleanup_list);

		for (i = 0; i < YAFFS_N_TEMP_BUFFERS; i++) {
			yaffs_free(dev->temp_buffer[i].buffer);
